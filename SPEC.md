...
- **No comparison to other people, ever, in any form — a hard law, not an omission.** No leaderboards, no "you're doing better than X% of users," no aggregate benchmarks of any kind, now or later. State this as an absolute rule rather than something that just happens to be missing, so it survives future feature pressure.
- **Comfortable with silence — no escalating win-back campaigns.** Most retention-optimized products treat a quiet user as a problem to solve with increasingly urgent "we miss you" pushes. This app doesn't escalate. The absence check-in (already spec'd) fires once, gently, and then genuinely stops — silence from a user is allowed to just be silence.
- **The buddy is allowed to say "I don't know."** Not liability language — genuine humility as a personality trait. Deferring to a real doctor, or plainly admitting a limit, is more trustworthy than an assistant performing omniscience, and it reinforces every other honesty commitment in this spec.
...
- **Forward-looking pattern nudges ("Tier 1" predictive) — statistics over the person's own history, not a model, and not gated by device tier.** This is genuinely novel — no competitor does proactive behavioral prediction, they're all reactive — and it's just an extrapolation of data already being tracked, not new architecture. Three concrete kinds, all routed through the arbitration layer like every other message type (not exempt from the one-per-day rule):
  - **Day-of-week pattern:** "based on the last 6 Tuesdays, dinner tends to land late and the day finishes short of the fiber target — want a heads-up earlier today?"
  - **Trend extrapolation toward the weight/activity goal:** "at your pace over the last 3 weeks, you're tracking toward about X% by day 90, a bit under/over the range you set — nothing to act on, just flagging it." Never framed as a guarantee, always as a current-trajectory observation.
  - **Lapse-risk detection:** "you've missed logging the last two Saturdays — want Saturday nudges to look different, or off entirely?" A question, not a guilt trip — offers to change the app's behavior, not just points out the person's behavior.
  - **This is exactly what the "predicting an individual's glucose response without a CGM" idea (rejected earlier — see the original scoring-formula decision) must never become.** Tier 1 predicts the person's own logged behavior and trajectory, which is honest arithmetic over real data. It never predicts a physiological response (blood sugar, insulin) without the real device data that would make such a prediction honest. Keep this line firm as the feature evolves.
- **Minimum sample size guardrail, wrapping around all of the above (including Tier 1).** Any pattern callout, correlation, trend extrapolation, or target-adjustment suggestion needs a minimum number of data points before it's allowed to surface at all — otherwise this becomes false-pattern claims dressed up as insight, exactly the overclaiming this app is built to avoid. Set an explicit threshold (e.g. don't surface a correlation or trend claim from fewer than ~2-3 weeks of relevant data) rather than leaving this to judgment call by call.

**A permanent, always-visible "enough for today" tap — a proud interaction, not just a reactive card.** Distinct from the reset-day moment (which is triggered by the app noticing absence or a wide miss): this is a standing, one-tap affordance on the Today screen itself, available every single day whether or not anything's gone wrong. Tap it, and the app asks nothing more for the rest of the day — no further nudges, no explanation required — and tomorrow starts fresh. Critically, tapping it must not count as a miss anywhere: it's excluded from lapse-risk detection (task 19) and shown neutrally, not negatively, in the consistency view. Most habit apps are built to make opting out effortful; making it easy, proud, and always available is the app's name rendered as an actual interaction rather than just copy.

**Settings as conversation, not a control panel — Phase 2, once buddy chat exists.** A menu full of individual toggles (per-nudge-type switches, a manual pause mode, a static chattier/quieter dial) is itself a small betrayal of "Enough" — decision fatigue dressed up as respecting autonomy. The more honest fix: day-to-day, relational adjustments ("go quieter on Saturdays," "that was a work trip, don't count it against anything") get said directly to the buddy, which remembers and adjusts — the way telling an actual friend something works, not the way negotiating with a preferences screen does. Structural, one-time setup choices (Health Connect connection, estimate calibration, dietary restrictions, delete my data) stay in a traditional Settings screen, since those aren't relational — only the adaptive, changeable-mood stuff moves to conversation.

**Buddy context transparency, in plain language — Phase 2.** Since the buddy's "memory" is really just assembled context fed into the model each session (recent logs, stated why, current focus lever, anything it's been told to adjust), that context should be visible and clearable on request — but as a plain-language sentence ("I know you're focusing on fiber, mentioned wanting more energy for your kids, and asked me to go quiet on Saturdays"), never a raw data dump that reads like a debug panel. A JSON export nobody reads is trust theater, not transparency.

**Custom quiet hours, not an assumed day/night pattern.** The walk-nudge and reset-moment logic currently assume a standard schedule — a shift worker's "evening" might be 8am. Let someone define their own quiet window in Settings; cheap to build, and stops the app from quietly excluding a real population.

**Hide-the-numbers mode must be consistent everywhere, not just on the home screen.** Show trend direction only (up/down/steady) without literal weight or fiber numbers, if chosen — and this has to apply to the exportable milestone card and the pre-visit report too, or someone could unknowingly share a number they specifically chose to hide. This operationalizes the eating-disorder safety principle in §0.6 as an actual usable setting, not just a background constraint.
...
RulesEngineState += currentFocusLever (fiber/walks/sleep/stress), minutesSinceLastMeal, onboardingCompletedDate, hasSeenMilestoneReview (bool), lastCheckpointNudgeDate, lastMilestoneIntervalShown (90/180/365), hasRespondedToOutcomesPing (bool), lastResetMomentShown (date), daysSinceLastLogAtLastCheck, lastProactiveMessageDate (enforces one-per-day arbitration), lastProactiveMessageType, consecutiveWeeksOverTarget, consecutiveWeeksUnderTarget, lastDayOfWeekNudgeShown, lastTrendExtrapolationShown, lastLapseRiskNudgeShown, enoughForTodayTappedDates (list of dates, excluded from lapse-risk detection and shown neutrally in consistency view)
UserGoal += quietHoursStart, quietHoursEnd (custom, no assumed day/night pattern), hideNumbersMode (bool — applies consistently to home screen, milestone export, and pre-visit report)
StressCheckIn
...
ConversationalPreference
  id, requestText (what the person actually said, e.g. "go quieter on Saturdays"), effectiveDate, expiryDate (nullable — some are one-time like excluding a trip, others standing), parsedAdjustment (which rule/nudge type it affects) — lets the buddy honor day-to-day requests without a settings-menu toggle for each one
```
